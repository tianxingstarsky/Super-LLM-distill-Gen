...
- 审核视图**两态**：默认按 **Markdown 渲染** 阅读（dsh 气泡规范，不可编辑）；
  点某条消息下方「✏️ 编辑第 N 条」进入**源码视图**（解除渲染、自动聚焦该条、可整段选中改写，
  正文与思考都可改），组件内一次「保存为新版本」，「取消编辑」返回渲染视图。
  源码视图是零构建的自定义组件（`lib/components/editable_chat/`）。
- 保存生成**新版本样本**（新 sample_id `xxx-r1`，原记录保留待判）——与内容哈希绑定一致，
  旧审核票不会套用到新内容。

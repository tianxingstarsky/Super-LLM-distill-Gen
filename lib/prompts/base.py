...
def registry_versions() -> Dict[str, Dict[str, PromptSpec]]:
    ...
    from lib.prompts import (
        agent, distill, document, dpo, identity, judge, magpie, multimodal, revise, style,
        stylefix, translation, tuning, workflow,
    )

    all_specs: Dict[str, Dict[str, PromptSpec]] = {}
    for mod in (magpie, distill, judge, translation, identity, document, tuning, style, multimodal, dpo, agent, stylefix, revise, workflow):
        for name in dir(mod):
